...
def format_phone_fast2sms(phone: str) -> str:
    """Ensure 10-digit format for Fast2SMS"""
...
def format_phone_e164(phone: str) -> str:
    """Ensure E.164 format (+91XXXXXXXXXX) for Twilio"""
    clean = "".join(filter(str.isdigit, phone))
...
def send_otp_sms(phone: str, otp: str, role: str = "farmer") -> dict:
    ...
    if SMS_PROVIDER == "fast2sms" and FAST2SMS_API_KEY and FAST2SMS_API_KEY != "your_fast2sms_api_key_here":
        ...
        print(f"\n🔧 [DEBUG] SMS_PROVIDER={SMS_PROVIDER}")
        print(f"🔧 [DEBUG] FAST2SMS_API_KEY={FAST2SMS_API_KEY[:8]}...{FAST2SMS_API_KEY[-4:]} (length={len(FAST2SMS_API_KEY)})")
        print(f"🔧 [DEBUG] Target phone: {target_phone}")
        try:
